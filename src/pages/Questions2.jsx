...
function Questions2() {
  ...
  const [royaltiesValue, setRoyaltiesValue] = useState("");

  useEffect(() => {
    const storedAmount = localStorage.getItem("royalitiesAmount");
    if (storedAmount) {
      setRoyaltiesValue(parseFloat(storedAmount)); // Parse the stored value to a numeric type
    }
  }, []);

  const handleNext = () => {
    // Set empty input to "0"
    const royalitiesAmount = royaltiesValue !== null && royaltiesValue !==  0  ? royaltiesValue : "";
    // Save to local storage with function name
    localStorage.setItem("royalitiesAmount", royalitiesAmount);
      ...
      <div className="w-100 d-flex justify-content-center">
        <div>
          ...
          <div className="d-flex justify-content-center flex-column align-items-center">
            <h3 className="fw-bold mt-4 mb-3">Royalties received</h3>
            <div className="input-div w-100 d-flex justify-content-start border align-items-center px-3 ">
              ...
              <InputNumber
                ...
                placeholder="Enter amount of Royalties received"
                value={royaltiesValue}
                onValueChange={(e) => setRoyaltiesValue(e.value)}
                locale="en-us"
                minFractionDigits={2}
              />
            </div>
